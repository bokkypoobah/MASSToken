...
echo "var pstOutput=`solc --optimize --combined-json abi,bin,interface $PRESALETOKENTEMPSOL`;" > $PRESALETOKENJS

echo "var mtOutput=`solc --optimize --combined-json abi,bin,interface $MASSTOKENTEMPSOL`;" > $MASSTOKENJS

echo "var stOutput=`solc --optimize --combined-json abi,bin,interface $STANDARDTOKENTEMPSOL`;" > $STANDARDTOKENJS
...
var pstAbi = JSON.parse(pstOutput.contracts["$PRESALETOKENTEMPSOL:PreSaleToken"].abi);
var pstBin = "0x" + pstOutput.contracts["$PRESALETOKENTEMPSOL:PreSaleToken"].bin;
...
var stAbi = JSON.parse(stOutput.contracts["$STANDARDTOKENTEMPSOL:StandardToken"].abi);
var stBin = "0x" + stOutput.contracts["$STANDARDTOKENTEMPSOL:StandardToken"].bin;

// console.log("DATA: mtpsAbi=" + JSON.stringify(mtpsAbi));
// console.log("DATA: pstAbi=" + JSON.stringify(pstAbi));
